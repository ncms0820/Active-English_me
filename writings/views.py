from writings.models import Writing
from users.models import User
from django.http.response import Http404
from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import status
from .serializers import WritingSerializer
# Create your views here.


class WritingList(APIView):
    def post(self, request, format=None):
        data = request.data
        user = User.objects.get(pk=data['user'])
        Writings = Writing.objects.filter(user=user)
        serializer = WritingSerializer(Writings, many=True)
        return Response(serializer.data)

class AddWriting(APIView):
    def post(self, request, format=None):
        data = request.data
        serializer = WritingSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(status=status.HTTP_400_BAD_REQUEST)


class WritingDetail(APIView):
    def get_object(self, pk):
        try:
            return Writing.objects.get(pk=pk)
        except Writing.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        writing = self.get_object(pk)
        serializer = WritingSerializer(writing)
        return Response(serializer.data)
        
    def put(self, request, pk, format=None):
        writing = self.get_object(pk)
        data = request.data
        serializer = WritingSerializer(writing, data=data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def delete(self, request, pk, format=None):
        writing = self.get_object(pk)
        writing.delete()
        return Response(status=status.HTTP_200_OK)